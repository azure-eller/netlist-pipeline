...
2. **General cross-section solver.** `next`. `fields.solve_cut(cut)`: N conductors and a
   permittivity map built from the cut, every conductor driven in turn, capacitance matrix
   from the energy matrix, target `z0` with neighbours grounded and capacitive coupling to each
   neighbour by net. `SOLVER_VERSION fd2d-0.2`, validated against `solve(Geometry)` on a bare
   microstrip cut. This is the step that makes the label geometry-aware.
3. **Board supply.** `later`. `scripts/harvest.py github`: `gh api search/code
...
6. **Frozen datasets, baseline, FNO, geometry-native judge.** `later`. `datasets.kind =
   'windows'`: a frozen list of window hashes with labels under one recipe, split by
...
| solver | `fd2d-0.1` | `windows.solver_version` |
