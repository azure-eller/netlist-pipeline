...
def box(ax, x1, y1, x2, y2, title, body="", fc=BLUE_FC, ec=BLUE, ts=9, bs=7.2, lw=1.3, pad=0.3):
    ...
    cx = (x1 + x2) / 2
    if body:
        ax.text(cx, y2 - 2.2, title, color=TXT, ha="center", va="top", fontsize=ts, weight="bold")
        ax.text(cx, y1 + (y2 - y1 - 3.2) / 2, body, color=SUB, ha="center", va="center", fontsize=bs)
    else:
        ax.text(cx, (y1 + y2) / 2, title, color=TXT, ha="center", va="center", fontsize=ts, weight="bold")
...
def frame(ax, x1, y1, x2, y2, title, ec, fc="none", ts=10):
    ...
    ax.text(x1 + 1.5, y2 - 1.2, title, color=TXT, ha="left", va="top", fontsize=ts, weight="bold")
...
def label(ax, x, y, s, color=SUB, size=7, ha="center", va="center", rot=0):
    ax.text(x, y, s, color=color, ha=ha, va=va, fontsize=size, rotation=rot, family="monospace" if "/" in s or "_" in s else None)
...
def system() -> None:
    ...
    box(ax, 30, 78, 66, 90, "api  (FastAPI)", "POST /designs → store upload, insert run + first job\nGET /runs/{id} → the rows\nGET /runs/{id}/artifacts/{name} → presigned redirect", bs=6.2)
    box(ax, 76, 78, 106, 90, "Postgres", "designs · runs · jobs · stages · candidates\nverifications · artifacts\nmodels · judge_approvals", fc=ORANGE_FC, ec=ORANGE)
    ...
    label(ax, 71, 85.8, "run + first job", size=6.2)
    arrow(ax, (47, 78), (136, 78), rad=0.22)
    label(ax, 91, 71.5, "upload bytes", size=6.5)

    # row B: worker
    frame(ax, 3, 38, 157, 66, "worker  — one job at a time, each in a child interpreter", BLUE, fc="#111827")
    box(ax, 6, 44, 30, 58, "job loop", "claim a queued job\n(SKIP LOCKED)\nrun its stage\nenqueue the next stage")
    frame(ax, 33, 41, 154, 61, "the eight stages: one jobs row and one stages row each, chained by sha256", "#2f4f6f", fc="#0f1a2e", ts=8.5)
    stages = [
        ("1  extract_netlist", "kicad-cli"), ("2  constraints", "net classes + patterns"),
        ("3  build_board", "pcbnew"), ("4  place", "simulated annealing"), ("5  route", "Freerouting"),
        ("6  judge", "the model slot"), ("7  verify", "DRC + netlist parity"), ("8  export", "gerbers, drill, render"),
    ]  # fmt: skip
    xs = []
    for i, (t, b) in enumerate(stages):
        ...
        slot = i == 5
        box(ax, x1, 45, x1 + 13.25, 55, t, b, fc=GREEN_FC if slot else "#1b2d4f", ec=GREEN if slot else BLUE, ts=7.6, bs=6.2)
        if i:
    ...
    box(ax, 4, 8, 34, 30, "the registry  (rows in Postgres)", "models: name, version, dataset,\nartifact key + sha256, feature\nencoding, sklearn, commit\n\njudge_approvals: name, version,\nartifact sha, golden-set sha\n\nboth immutable", fc=GOLD_FC, ec=GOLD, bs=6.6)
    frame(ax, 40, 3, 157, 33, "judge service  (JUDGE_URL, one versioned judge per process)", GREEN, fc="#0f1a14")
    box(ax, 44, 10, 78, 26, "rule judge", "IPC-2141 impedance\nIPC-2221 current capacity\ndecoupling · crosstalk · vias\nscore = −Σ weighted excess", bs=6.6)
    frame(ax, 83, 6, 154, 29, "physics provider  (pluggable: where the impedance numbers come from)", "#2e6b3a", fc="#0d1a12", ts=8)
    box(ax, 86, 10, 106, 23, "formula", "IPC-2141 closed form\nmicroseconds", ts=8, bs=6.4)
    box(ax, 109, 10, 129, 23, "oracle", "our 2D field solver\n0.36 s per geometry", ts=8, bs=6.4)
    box(ax, 132, 10, 152, 23, "learned-fd v5", "trained on the oracle\nmicroseconds, 0.9% error", fc=GOLD_FC, ec=GOLD, ts=8, bs=6.4)
    arrow(ax, (78, 18), (86, 18))
    label(ax, 82, 22.6, "z0 of every\nhigh-speed net", size=5.8)

    # the slot, the gate, the bytes
    sx = (xs[5][0] + xs[5][1]) / 2
    arrow(ax, (sx, 45), (sx, 33), color=GREEN, lw=1.6)
    label(ax, sx + 1.2, 39, "POST /v1/score  (board, netlist, constraints)\n← score, violations, judge {name, version, artifact sha}", color="#8fbf8f", size=6.3, ha="left")
    poly(ax, [(xs[5][0] + 1, 45), (xs[5][0] + 1, 36), (19, 36), (19, 30)], color=GOLD)
    label(ax, 20, 33, "gate: (name, version, artifact sha)\nmust be a judge_approvals row", color=GOLD, size=6.3, ha="left")
    poly(ax, [(34, 14), (38, 14), (38, 1.5), (142, 1.5), (142, 10)], color=GOLD)
    label(ax, 90, 0.2, "the models row names the artifact key; the service refuses bytes whose sha256 differs", color=GOLD, size=6.3, va="bottom")
